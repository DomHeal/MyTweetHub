...
import com.mytweethubapplication.Login.UserData;
...
public class Timelines extends Application {
	...
	public static void getTimeLine() throws IllegalStateException, MalformedURLException{
		...
		LoginGUI.twitter = LoginGUI.getTwitter();

		/* Creates and Displays Public Time */
		try {
			statusList = LoginGUI.twitter.getHomeTimeline(new Paging().count(200));
	...
	public static void getUserTimeLine() throws IllegalStateException{
		...
		try {
			final List<Status> statusList2 = LoginGUI.twitter.getUserTimeline(new Paging().count(200));
		...
		/* Listener for Popup Menu and Get Selected row */
			...
			mntmDelete.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					try {
						LoginGUI.twitter.destroyStatus(tweetid);
						dlm2.remove(row);
